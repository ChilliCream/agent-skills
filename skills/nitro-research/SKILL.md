...
4. **Install the role agents** once per project, if your harness needs definitions to control model and effort (Claude Code does; see the harness reference). Then **open the epic.** Create it with `--type epic --label research`, take its id from `--output json` (never construct ids), create the folder named after it, and then the scout step (for a lookup, the single reader step instead). Put the folder path in the epic description.
