# GraphQL Error Handling — Stage 6a Pattern

The standard error pattern for all mutation domain errors.
From Marc-Andre Giroux's [Guide to GraphQL Errors](https://productionreadygraphql.com/2020-08-01-guide-to-graphql-errors).
...
### Domain Errors → Stage 6a on the mutation payload (in schema)
...
- **Always model these in the schema using Stage 6a**

---

## The Stage 6a Pattern

Stage 6a = **Payload wrapper** + **`errors` union list** + **Error interface**
...
The shared interface is what makes Stage 6a forward-compatible. Define it once;
