...
- **Fixer**: applies the verified plan exactly, nothing more. Then re-review. Cap at 3 cycles, then escalate to the planner.

Roles are capabilities, not model names. For Claude Code and Codex the mapping is settled: read [references/claude-code.md](references/claude-code.md) or [references/codex.md](references/codex.md) for the models and effort per role and for the spawning, isolation, and wake mechanics. On any other harness, match these classes: implementer and fixer on a Sonnet-class model at high effort; the reviewer on an Opus-class model at medium effort; the verifier and the planner on a Fable-class model at medium effort.
...
1. Check for a rival first: `nitro agent list --role orchestrator --output json` names the actors this workspace knows in that role, so any live hit is another orchestrator: do not take the role; register without it (`nitro agent register --actor <name> --role standby`), broadcast the clash to the planner (`nitro agent mail broadcast --role planner --actor <name> --subject "second orchestrator" --body "<the live actor you found>"`), and stop until the planner rules. Only with no hit take the role: `nitro agent register --actor <name> --role orchestrator`. `<name>` is the actor your session context states, or one from `nitro agent login`; never invent it, and repeat `--role orchestrator` on each register, since omitting it writes an empty role. Planners find you by role, so the role is the part that must be right.
...
1. Group ready tasks into waves by **area label** (`nitro agent tasks ready --label <area> --output json` is one wave's worklist). Unlabeled ready tasks go back to the planner. Tasks labelled `wayfinder:*` or `research*` belong to other sessions: skip them in every listing, never claim, close, or epic-close them.
2. At wave launch, claim the wave's tickets (`nitro agent tasks update <ids...> --claim --actor <name>`); release a skipped one with `--status open --assignee ""`; park an escalated one with `--status blocked --assignee ""` after commenting the blocker, so it leaves `ready` until the planner rules and reopens it. Inside a wave: tickets run strictly one at a time (shared files, shared verification).
...
When you are stuck, need more input, or need something clarified, the planner is the escalation target. The planner owns the conversation with the user; never route around it to ask the user yourself. The only exception is a missing `nitro` CLI, which leaves no channel at all. Escalate in this order: comment the blocker on the ticket, park it with `nitro agent tasks update <id> --status blocked --assignee "" --actor <name>`, then mail: reply on the originating briefing thread when one exists, otherwise `nitro agent mail broadcast --role planner --actor <name> --subject "<task-id>: <blocker>" --body "..."`.
...
- Never let one stuck ticket stall a wave: comment the blocker, set the ticket `blocked`, mail the planner, continue. The planner reopens it after ruling; you never reopen it yourself.

## Closing discipline

- Anything labelled `wayfinder:*` (planning maps and their decision tickets) or `research*` (research epics and their steps) belongs to other sessions: never claim, close, or epic-close it.
...
- One authenticated browser via CDP for live verification. The user logs in interactively (request the login through the planner); agents connect read-only, never enter credentials, never log out. Poll for real session validity (a protected route loading), not cookie presence.
...
Launch wave → process completion (close passes, fix-or-escalate failures) → record decisions → launch next wave(s). Between waves, promote any standing rule the wave produced into memory, then reconcile the tracker (`nitro agent tasks ready --output json`, `nitro agent tasks dep cycles --output json`, close each finished implementation epic via `nitro agent tasks epic status --output json`, duplicates from parallel planners) and drain the mailbox. Do not use `epic close-eligible`: it would also close a finished wayfinder map or research epic. The run ends when `nitro agent tasks ready --output json` holds no labeled implementation task the orchestrator can still schedule; wayfinder and research tasks never count, and escalated tickets stay open with the planner. End with a final report of what shipped, the escalated tickets with their mail threads, and the compiled deferred-decisions list.
